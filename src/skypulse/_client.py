...
def _safe_fetch_sync(func: Any) -> Tuple[Any, Exception | None]:
    try:
        return (func(), None)
...
class SkyPulseClient(_BaseClient):
    ...
    def prefetch(
        ...
        *,
        ...
        """Fetch all weather data for a location sequentially."""
        rlat, rlon = self._resolve_coords(city, lat, lon, auto_locate)
        ...
        results = [
            _safe_fetch_sync(lambda: self.get_current_weather(lat=rlat, lon=rlon, skip_cache=skip_cache)),
            _safe_fetch_sync(lambda: self.get_forecast(lat=rlat, lon=rlon, skip_cache=skip_cache)),
            _safe_fetch_sync(lambda: self.get_air_quality(lat=rlat, lon=rlon, skip_cache=skip_cache)),
            _safe_fetch_sync(lambda: self.get_air_quality_forecast(lat=rlat, lon=rlon, skip_cache=skip_cache)),
            _safe_fetch_sync(lambda: self.get_uv_index(lat=rlat, lon=rlon)),
            _safe_fetch_sync(lambda: self.get_uv_forecast(lat=rlat, lon=rlon)),
            _safe_fetch_sync(lambda: self.get_magnetic_storm()),
            _safe_fetch_sync(lambda: self.get_magnetic_forecast()),
        ]
